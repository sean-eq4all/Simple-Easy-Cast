...
**목적**: 자막 오버레이 테스트, 위치·길이·속도·쉬운말 재변환, 더빙 동기 재생

#### 3패널 레이아웃

```
┌──────────────────────────────────────────────────────────────────┐
│ 툴바: [← 뉴스 상세] | 저장 | 되돌리기 | 다시실행 | 자막 보이기 ☑ | Export | 송출 │
├───────────────────────────────┬──────────────────────────────────┤
│                               │  자막 속성 패널                    │
│   비디오 프리뷰                │  ┌ 원문 (STT) ───────────────┐  │
│   ┌─────────────────────┐    │  │ ...                        │  │
│   │  [영상 + 자막 오버레이] │    │  └──────────────────────────┘  │
│   │  드래그로 위치 이동     │    │  ┌ 쉬운말 (편집 가능) ────────┐  │
│   └─────────────────────┘    │  │ ...                        │  │
│   ▶ ⏸  00:01:23 / 00:05:00   │  └──────────────────────────┘  │
│   [━━━━●━━━━━━━━━━] 시크 바   │  위치 X [___] Y [___]            │
│                               │  표시 구간 In [___] Out [___]     │
│                               │  속도 [1.0 ───●── 1.1]           │
│                               │  [원 구간 길이에 맞추기]           │
│                               │  [쉬운말 재변환] [짧게 출력 재변환] │
│                               │  [TTS 미리듣기]                   │
...
│ 타임라인 (줌: - / +)                                              │
│ Video  [████████████████████████████████████████████████]          │
│ Subtitle [blk1][blk2───][blk3] ...  ← 드래그로 길이·위치 조정      │
│ TTS/Audio  ∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿∿  │
│ Playhead ▲                                                       │
└──────────────────────────────────────────────────────────────────┘
```
...
| 영역 | 상호작용 |
|------|----------|
| 툴바 자막 보이기 | 체크박스/토글로 프리뷰·타임라인 자막 오버레이 표시 on/off |
| 비디오 프리뷰 | 자막 박스 드래그 → 위치 변경; 클릭 시 해당 세그먼트 선택 |
| 시크 바 | 임의 위치 seek; 드래그 스크럽 시 TTS 프리뷰 |
| 재생 컨트롤 | 재생/일시정지, Space 단축키 |
| 자막 속성 패널 | 선택 세그먼트의 원문·쉬운말, in/out, 속도 편집 |
| 속도 슬라이더 | 1.0~1.1 제한; 변경 시 타임라인 블록 길이 시각 반영 |
| 쉬운말 재변환 | 쉬운말 변환 API 재호출 → 결과 미리보기 → 수락 시 `easyText` 갱신·TTS 재합성 |
| 짧게 출력 재변환 | 쉬운말 변환 API 재호출(짧게 출력 옵션) → 원문보다 짧은 `easyText` 제안 → 수락 시 갱신·TTS 재합성 |
| TTS 미리듣기 | 현재 세그먼트 더빙만 재생 |
| 타임라인 | 블록 클릭 선택; 좌우 핸들로 길이; 블록 드래그로 시간 이동; 빈 영역 클릭 seek |
...
- 문장 분할 없이 단일 세그먼트 내 텍스트만 치환

#### 자막 선택 동기화

- 타임라인 블록 선택 ↔ 프리뷰 자막 하이라이트 ↔ 속성 패널 필드 동기화
- 재생 중 현재 playhead가 지나는 세그먼트 자동 하이라이트(선택)
...
sequenceDiagram
  ...
  Admin->>Editor: 위치·길이·속도·쉬운말 재변환 조정·저장
...
4. **편집 툴** → 자막·속도·쉬운말 재변환(필요 시 짧게 출력) 조정 → 저장
5. **Export** → 단일 영상 생성
6. **송출** → 방송 시스템 전송
...
| 원칙 | 설명 |
|------|------|
| 데스크톱 우선 | 타임라인·3패널 편집기는 최소 1280px 너비 가정 |
...
| 쉬운말 변환 | §3.5(변환 단계) + §3.6 속성 패널 쉬운말 |
| 자막 테스트 편집 툴 | §3.6 편집 툴 전체 |
| 위치·길이 조정 | §3.6 프리뷰 드래그, 타임라인 블록 |
| 속도 최대 1.1배 | §3.6 속도 슬라이더 |
| 긴 문장 재치환 | §3.6 [쉬운말 재변환]·[짧게 출력 재변환] |
| 더빙 재생 | §3.6 재생·시크·TTS 트랙 |
| Export | §3.7 Export |
